...
public class RegularPaymentControllerImplTest {
    ...
    private final RegularPayment regularPayment = RegularPaymentUnitTestGenerator.populateRegularPayment();
    private final RegularPaymentResponseDto regularPaymentResponseDto = RegularPaymentUnitTestGenerator.populateRegularPaymentResponseDto();
    private final RegularPaymentRequestDto regularPaymentRequestDto = RegularPaymentUnitTestGenerator.populateRegularPaymentRequestDto();
    ...
    void create_ShouldReturnRegularPaymentResponseDto_WhenRegularPaymentRequestIsCorrect() {
        Mockito.when(regularPaymentMapper.toRegularPayment(regularPaymentRequestDto)).thenReturn(regularPayment);
        Mockito.when(regularPaymentMapper.toDto(regularPayment)).thenReturn(regularPaymentResponseDto);
        Mockito.when(regularPaymentService.create(regularPayment)).thenReturn(regularPayment);

        assertEquals(regularPaymentResponseDto, regularPaymentController.create(regularPaymentRequestDto));
    }
