...
public class PassportServiceImpl implements PassportService {
  ...
  private void validatePassport(final Passport passport) {
    ...
    if (dateSupportService.checkIfDateIsLaterThanToday(dateIssue)) {
      throw new ValidationException(String.format("Expiration date is incorrect: %s", dateIssue));
    }

    if (!dateSupportService.checkIfDateIsLaterThanToday(terminationDate)) {
      throw new ValidationException(String.format("Expiration date is incorrect: %s", terminationDate));
    }
    if (!(dateSupportService.checkIfTwoDatesAreEqual(dateIssue.plusYears(PASSPORT_VALIDITY), terminationDate))) {
      throw new ValidationException(
          String.format("Registration date and expiration date must have exactly %d years difference",
              PASSPORT_VALIDITY));
    }
    if (dateSupportService.checkIfDateIsLaterThanToday(birthDate.plusYears(PASSPORT_OWNER_MINIMAL_AGE))) {
      throw new ValidationException(
          String.format("Passport owner should be at least %d years old", PASSPORT_OWNER_MINIMAL_AGE));
    }
  }
}
